...
- 转录质量核对：时间轴/分块证据、可选 CPU 语音覆盖检测和识别诊断；逐项回听、确认或局部重识别，采纳生成新版本。检查结果不代表准确率。
...
v0.5.0 使用 SQLite schema 5。升级前退出应用并备份资料目录；旧版程序不能直接打开升级后的资料库。回退时需恢复升级前备份，不要手工修改 schema 版本号。
...
v0.5.0 增加逐项核对、可选 CPU 语音覆盖检测、局部识别候选和精简阅读侧栏；151 项自动测试及本机验证见[质量与交互验收](docs/validation-quality-ux-0.5.0.md)。
