...
面向个人的本地课程资料库。把 B 站单视频、选定分 P、本地音视频或字幕，整理成可校对、可检索、带出处的学习材料。

**Windows 优先 · Rust + Tauri 2 + React · SQLite · 独立 Whisper worker**

有可读取字幕时直接提取，**不需要下载视频或音轨**。没有可用字幕时仅获取音轨并转写。首次读取课程后默认只选第一 P；整套课程必须由使用者明确选择。
...
4. 导入 B 站链接后核对分 P 和时长，点击“开始处理”。仅提取字幕模式不会自动下载音轨。
...
识别进程与桌面应用分离，可复用已有 Python 环境。需要 `openai-whisper`、`torch`、`numpy`；下载 B 站音轨还需要 `yt-dlp`。FFmpeg 与 ffprobe 通过独立可执行文件配置。
...
首版支持 CPU 和 NVIDIA CUDA；其他 GPU 后端、跨课程语义检索、实时转写和更多网站未纳入首版。B 站接口可能要求登录或受网络限制。没有提供 API 密钥时，云端模型的实际回答质量未被验证。
